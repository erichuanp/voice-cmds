...
- 开机自启动（debug 模式锁定）
- **关于页**：软件版本、开源声明（MIT + 署名与邮件通知要求）、Releases / 作者主页 / 建议邮箱链接；「检查更新」查询 GitHub Releases 最新版：
  - 一致 → 绿色“已经是最新版本”；网络失败 → 红色“网络出错检查失败”
  - 主版本号（X.Y.Z 的 X）不一致 → 红色“版本过时，请于GitHub Release下载最新版本”（不可热更）
  - 同主版本有新版 → “更新到最新版本”按钮（§8.4 差分热更新）

托盘菜单：**设置 / 帮助 / 定时任务 / 退出**。「帮助」为独立窗口，列出当前热键、结束方式、全部内置命令、定时任务语法、打开/自定义命令与匹配规则；「定时任务」打开任务列表（§7.3）。

### 8.4 差分热更新（`updater.py`）

- **发布产物**：`build_release.ps1` 生成 `manifest.json`（每个随包文件的 SHA256+大小），随安装包/便携版内置（本地清单用于比对），并作为 GitHub Release 独立资产上传。
- **检查与分流**：见 §8.2 关于页（主版本不一致 → 手动下载；同主版本 → 热更新）。
- **差分下载**：拉取远端 manifest → 与本地清单比对 → 仅变更/删除文件处理；变更文件通过 zip 的 **HTTP Range** 请求按条目拉取（中央目录 + 各条目压缩字节区间），失败自动回退整包下载；逐文件 SHA256 校验。
- **应用**：暂存到 `<app>\_update\` + 计划文件 `_update.json`；点击“更新完成，程序将自动重启以应用更新”后，程序写出 `update.bat`（等待 2s 释放文件锁 → xcopy 覆盖 → 删除下线文件 → 重启 exe）并退出。安装版与便携版同样支持。
- 用户数据（models/logs/config/scripts）不参与比对、不被触碰；卸载器同时清理 `_update/`、`_update.json`、`update.bat`。
